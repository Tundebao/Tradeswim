...
const ManualTrade = () => {
  ...
  const validateStockForm = () => {
    ...
    if (!stockForm.quantity || parseInt(stockForm.quantity) <= 0) return "Valid quantity is required";
    if (stockForm.orderType === "limit" && (!stockForm.limitPrice || parseFloat(stockForm.limitPrice) <= 0)) {
  ...
  const validateOptionForm = () => {
    ...
    if (!optionForm.quantity || parseInt(optionForm.quantity) <= 0) return "Valid quantity is required";
    if (!optionForm.expirationDate) return "Expiration date is required";
    if (!optionForm.strikePrice || parseFloat(optionForm.strikePrice) <= 0) return "Valid strike price is required";
    if (optionForm.orderType === "limit" && (!optionForm.limitPrice || parseFloat(optionForm.limitPrice) <= 0)) {
      return "Limit price is required for limit orders";
    }
    return "";
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const tradePayload: Omit<Trade, "id" | "created_at" | "updated_at" | "status"> = {
        broker_account_id: parseInt(formData.brokerAccountId),
        symbol: formData.symbol.toUpperCase(),
        quantity: parseInt(formData.quantity),
        price: formData.orderType === "limit" ? parseFloat(formData.limitPrice) : 0,
        ...
        ...(tradeType === "option" && {
          option_details: {
            ...
            strike_price: parseFloat(optionForm.strikePrice),
      ...
      if (result.success) {
        ...
        if (tradeType === "stock") {
          ...
          setOptionForm(prev => ({ ...prev, symbol: "", quantity: "", limitPrice: "", expirationDate: "", strikePrice: "" }));
        }
      } else {
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="text-muted-foreground">
            Place manual trades for stocks and options
          </p>
        </div>
