...
import type { Position } from "@/types/broker"
import { fetchPositions } from "@/services/brokerService"
import { formatCurrency, formatPnL } from "@/utils/formatters"
import { AlertTriangle, ArrowDown, ArrowUp, Database } from "lucide-react"
    ...
    <AdminLayout>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-muted-foreground">View all open positions across your accounts</p>
        </div>
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-yellow-700">
                  <p>Connect a broker account to view your positions. Go to the Accounts page to add a broker.</p>
                </div>
                <div className="mt-4">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total Position Value</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Real-time view of all positions across accounts</CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
                  ...
                  {positions.map((position, index) => {
                    ...
                    return (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <Badge variant="outline" className="capitalize">
                            ...
                            {position.type === "option" && position.option_type && (
                              <span className="ml-1">({position.option_type})</span>
                            )}
                          </Badge>
                        </TableCell>
                        ...
                        <TableCell className={`text-right ${pnl.class}`}>{pnl.value}</TableCell>
              ...
              <div className="text-center py-12">
                ...
                <p className="mt-1 text-sm text-gray-500">You don't have any open positions at this time.</p>
              </div>
            ) : (
              <div className="text-center py-12">
                <Database className="mx-auto h-12 w-12 text-gray-400" />
                <h3 className="mt-4 text-lg font-medium">No broker connected</h3>
                <p className="mt-1 text-sm text-gray-500">Connect a broker to view positions.</p>
              </div>
            )}
          </CardContent>
        </Card>
